...
namespace Bot::BotHandler::YouTube::Media {
    ...
    ptrMessage MediaHandler::to_youtube_media(shared_ptr<BotHandlerContext> context, bool is_video) {
    ...
    bool MediaHandler::check(shared_ptr<BotHandlerContext> context) {
        return (
            context->access.full 
    ...
    ptrMessage MediaHandler::handle(shared_ptr<BotHandlerContext> context) {
    ...
    ptrMessage MediaHandler::send_audio_settings(shared_ptr<BotHandlerContext> context) {
        auto settings = context->global_context->youtube_audio_setting_repository->get_by_user_id(context->user->id);
        ...
        const string xlsx_path = fmt::format("{}/{}.xlsx", context->config->get_tmp_path(), context->user->id);
    ...
    ptrMessage MediaHandler::handle_audio_settings(shared_ptr<BotHandlerContext> context) {
        const string xlsx_path = fmt::format("{}/{}.xlsx", context->config->get_tmp_path(), context->user->id);
        context->bot->download_file(context->message->file_download_id, xlsx_path);
        ...
        for (unsigned short i = 1; sheet.contains({i, 2}) && sheet.contains({i, 3}); i++) {
            ...
            auto video_data = context->global_context->youtube_api->get_video(url);
        ...
        context->global_context->youtube_audio_setting_repository->update_by_user_id(context->user->id, std::move(result));
    ...
    ptrMessage MediaHandler::handle_urls(shared_ptr<BotHandlerContext> context) {
        ...
        while (getline(stream, buffer)) {
            ...
            auto result = context->global_context->youtube_api->get_video(buffer);
            if (result == nullptr) {
                return context->bot->send_message( {
                    .chat_id = context->chat->id,
