...
void JSONSTDOutLogger::info(string_view file, int line, string_view event, string_view message) const {
    if (_level <= EnumLoggerLevel::INFO) {
        print_event(cout, file, line, "INFO", event, message);
    }
}

void JSONSTDOutLogger::warning(string_view file, int line, string_view event, string_view message) const {
    if (_level <= EnumLoggerLevel::WARNING) {
        print_event(cout, file, line, "WARNING", event, message);
    }
}

void JSONSTDOutLogger::error(string_view file, int line, string_view event, string_view message) const {
    if (_level <= EnumLoggerLevel::ERROR) {
        print_event(clog, file, line, "ERROR", event, message);
    }
}

void JSONSTDOutLogger::debug(string_view file, int line, string_view event, string_view message) const {
